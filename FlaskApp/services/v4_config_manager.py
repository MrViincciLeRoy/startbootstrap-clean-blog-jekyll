"""
AI settings management for article generation - V4 Compatible
"""
import os
import json
...
class AISettingsManager:
    ...
    def __init__(self, config_file=None):
        ...
        # FIXED: Match V4 path structure (no FlaskApp prefix for V4 usage)
        self.github_path = os.path.join('FlaskApp', 'services', 'v4', 'config', 'ai_settings.json')
        
        # FIXED: Add search_config_path and update database_path to v4
        self.defaults = {
            'include_front_matter': True,
            'fetch_images': True,
            'embedding_model': 'all-MiniLM-L6-v2',
            'llm_model': 'LiquidAI/LFM2-1.2B-RAG',
            'config_path': 'research_v4/article_config.json',  # v4
            'database_path': 'research_v4/flora_data.db',  # v4
            'device': 'cpu',
            'load_in_8bit': False,
            'max_articles_per_run': 1,
            'search_config_path': 'research_v4/search_config.json'  # ADDED
        }
    ...
    def save_settings_to_github(self, settings, gh_manager, file_data=None):
        ...
        try:
            # Validate against defaults - ensure all required fields present
            validated_settings = {}
